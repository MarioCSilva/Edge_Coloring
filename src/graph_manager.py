...
class Graph_Manager:
	...
	def show_graph(self, G):
		...
		nx.draw(G, node_positions, edge_color=edge_colors,\
	...
	def save_graph(self, G, strategy=None):
		...
		if not strategy:
			...
			plt.savefig(f"{self.COLORED_GRAPH_DIRECTORY}{strategy}_Colored_Graph_{num_nodes}.png", bbox_inches='tight')
